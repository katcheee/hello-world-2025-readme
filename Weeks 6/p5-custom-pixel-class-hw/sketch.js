...
class ColorPixel {
  ...
  displayAsImage() {
    ...
    if (this.hsbValue >= 0 && this.hsbValue < 4) {
...
function setup() {
  ...
  // Resize all images
  red.resize(0, pixelSize);
  green.resize(0, pixelSize);
  blue.resize(0, pixelSize);
  yellow.resize(0, pixelSize);
  purple.resize(0, pixelSize);
  orange.resize(0, pixelSize);
  pink.resize(0, pixelSize);
  cyan.resize(0, pixelSize);

  // Calculate grid dimensions
  cols = floor(capture.width / gridSizeX);
  rows = floor(capture.height / gridSizeY);
